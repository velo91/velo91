...
$servers = [
    [
        "name" => "web-unit",
        "url"  => "https://103.197.191.92/index.php?range=".$range."&json=1&latest=1"
    ],
    [
        "name" => "UBBG server003",
        "url"  => "https://ejournal.bbg.ac.id/ide/atheos/monitoring/index.php?range=".$range."&json=1&latest=1"
    ],
    [
        "name" => "UNCM srv",
        "url"  => "https://siakad.uncm.ac.id/app/ide/atheos/monitoring/index.php?range=".$range."&json=1&latest=1"
    ]
];
...
if(!is_readable($db_path)) {
    die('Database monitoring tidak bisa diakses');
}
...
try {
    ...
    die('Gagal buka database monitoring: ' . $e->getMessage());
}

// Fungsi ambil data dan konversi waktu ke WIB
function get_data($db, $table, $where = '', $latest_only = false) {
    global $limit;
    $limit_sql = $latest_only ? 1 : $limit;
    $stmt = $db->query("SELECT created_at,value FROM $table $where ORDER BY created_at DESC LIMIT $limit_sql");
    ...
    return $latest_only ? ($rows[0] ?? null) : $rows;
}
// Ambil semua data
$cpu_data = get_data($db, 'instance_cpu', '', $latest_only);
$memory_data = get_data($db, 'instance_memory', '', $latest_only);
$disk_data = get_data($db, 'instance_disk_usage', "WHERE disk = '/'", $latest_only);
$load_data = get_data($db, 'instance_load_average', "WHERE period = 1", $latest_only);
// Hitung CPU core
...
<html lang="id">
<head>
    <?php if($show_manifest==true) {echo '<link rel="manifest" href="manifest.json">';} ?>
    <meta name="theme-color" content="#000000">
    <meta charset="UTF-8">
    <title>Login Akses</title>
    <link rel="icon" href="<?= $base_url ?>icon-192.png">
...
<body class="dark-mode">
    ...
    <div class="container-fluid text-center position-relative">
        ...
        <?php
        /*
        foreach ($row as $i => $srv): 
            $id = array_search($srv, $servers) + 1;
        ?>
        <div class="col-lg-6 col-12">
            <!-- BOX SERVER -->
            <div class="p-2 border rounded">
                <h5 class="text-center mb-3"><?= $srv['name'] ?></h5>
                <div class="row" id="chartGrid<?= $id ?>">
                    <div class="col-6 chart-box">
                        <h6>CPU <span id="status-cpu-<?= $id ?>"></span></h6>
                        <canvas id="cpuChart<?= $id ?>"></canvas>
                    </div>
                    <div class="col-6 chart-box">
                        <h6>RAM <span id="status-memory-<?= $id ?>"></span> <span id="status-swap-<?= $id ?>"></span></h6>
                        <canvas id="memoryChart<?= $id ?>"></canvas>
                    </div>
                    <div class="col-6 chart-box">
                        <h6>DISK <span id="status-disk-<?= $id ?>"></span></h6>
                        <canvas id="diskChart<?= $id ?>"></canvas>
                    </div>
                    <div class="col-6 chart-box">
                        <h6>BEBAN <span id="status-load-<?= $id ?>"></span></h6>
                        <canvas id="loadChart<?= $id ?>"></canvas>
                    </div>
                </div>
            </div>
        </div>
    ...
    function statusBadge(p) {
        if (p < 30)return`<span class="badge text-bg-secondary fs-6">Ringan (${p}%)</span>`; else if (p < 70)return`<span class="badge text-bg-success fs-6">Normal (${p}%)</span>`; else if (p < 90)return`<span class="badge text-bg-warning fs-6">Sibuk (${p}%)</span>`; else return`<span class="badge text-bg-danger fs-6">Overload (${p}%)</span>`;
    }
    function statusBadgeDisk(p) {
        if (p < 70)return`<span class="badge text-bg-success fs-6">Normal (${p}%)</span>`; else if (p < 90)return`<span class="badge text-bg-warning fs-6">Peringatan (${p}%)</span>`; else return`<span class="badge text-bg-danger fs-6">Kritis (${p}%)</span>`;
    }
    ...
    async function fetchServerData(server) {
        try {
            ...
            if (!res.ok) throw new Error(`Gagal fetch ${server.name}`);
            return await res.json();
        } catch (err) {
            console.error(`Error ${server.name}:`, err);
            return null;
        }
    }
    ...
    function renderServerCharts(server, data) {
        if (!data) return;
        const id = server.id;
        const cpu = parseFloat(data.cpu?.value ?? 0);
        const memory = parseFloat(data.memory?.value ?? 0);
        const disk = parseFloat(data.disk?.value ?? 0);
    ...
    //     if (!data) return;
    //     const id = server.id;
    //     const cpuValues = data.cpu.map(d => parseFloat(d.value));
    //     const memoryValues = data.memory.map(d => parseFloat(d.value));
    //     const diskValues = data.disk.map(d => parseFloat(d.value));
    //     const loadValues = data.load.map(d => parseFloat(d.value));
    //     const labels = data.cpu.map(d => d.created_at_wib || d.created_at);
    //     const swap = parseFloat(data.swap ?? data.swap_percent ?? 0);
    //     const labelFiltered = downsample(labels, POINTS);
    ...
    //     const cpuCore = data.meta?.cpu_core ?? 1;
    //     createChart(`cpuChart${server.id}`, "CPU", labelFiltered, cpuFiltered, 0, 100, '%', 'blue', 0, server.name);
    //     createChart(`memoryChart${server.id}`, "RAM", labelFiltered, memoryFiltered, 0, 100, '%', 'orange', 0, server.name);
    //     createChart(`diskChart${server.id}`, "DISK", labelFiltered, diskFiltered, 0, 100, '%', 'green', 0, server.name);
    //     createChart(`loadChart${server.id}`, "BEBAN", labelFiltered, loadFiltered, 0, Math.max(cpuCore, maxLoad), '', 'purple', 0, server.name);
    //     $(`#status-cpu-${id}`).html(statusBadge(cpuValues.at(-1) ?? 0));
    //     $(`#status-memory-${id}`).html(statusBadge(memoryValues.at(-1) ?? 0));
    //     $(`#status-swap-${id}`).html(`SWAP ${swap}%`);
    //     $(`#status-disk-${id}`).html(statusBadgeDisk(diskValues.at(-1) ?? 0));
