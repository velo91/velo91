...
# --- Baca log Auth dan ambil IP ---
ips = []
ip_pattern = re.compile(r'(\d{1,3}(?:\.\d{1,3}){3})')  # regex IP v4
with open(LOG_FILE, encoding='utf-8', errors='ignore') as f:
#with open(LOG_FILE) as f:
    for line in f:
        if "sshd" in line:  # hanya log SSH
...
print(f"💻 Daftar IP terbanyak di LOG ERROR nginx (0>{LOW_ACTIVITY_THRESHOLD}>{HIGH_ACTIVITY_THRESHOLD}). Last update {formatted_time} {timezone}.")
